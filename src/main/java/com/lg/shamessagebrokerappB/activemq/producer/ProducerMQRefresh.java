...
public class ProducerMQRefresh extends TimerTask {
	...
	public void run() {
		...
		if (!mqService.getIsAlive().equals(Boolean.TRUE)) {
			log.info("Message Broker B[ActiveMQ] Producer MQ is not up. Sending mail to support team.");
		} else {
			log.info("Message Broker B[ActiveMQ] Producer Connection working properly.");
		}
	}
}