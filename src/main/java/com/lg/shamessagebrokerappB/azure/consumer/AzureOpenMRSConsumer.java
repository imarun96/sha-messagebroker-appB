...
public class AzureOpenMRSConsumer {
    ...
    public void runConsumer() throws JMSException, NamingException, IOException, ParseException {
        try {
            ...
            log.info("Initializing Message Broker B[Azure] OpenMRSConsumer connection.");
            ...
            log.info("Creating Message Broker B[Azure] OpenMRSConsumer Context.");
            Context context = new InitialContext(env);
            log.info("Creating Message Broker B[Azure] OpenMRSConsumer ConnectionFactory.");
            ConnectionFactory connectionFactory = (ConnectionFactory) context.lookup("ServiceBusConnectionFactory");
            log.info("Creating Message Broker B[Azure] OpenMRSConsumer Connection.");
            connection = connectionFactory.createConnection();
            connection.start();
            log.info("Message Broker B[Azure] OpenMRSConsumer Connection started.");
            log.info("Creating Message Broker B[Azure] OpenMRSConsumer Session.");
            ...
            while (true) {
                ...
                if (msg instanceof TextMessage) {
                    ...
                    log.info(
                            "Queue[{}] is empty. Hence, closing the Message Broker B[Azure] OpenMRSConsumer connection.",
                            targetOpenMRSQueue);
                    break;
                }
            }
            ...
            log.error(
                    "Message Broker B[Azure] OpenMRSConsumer is not up. Please check the producer of the other end. {}",
                    e.getMessage());
        } finally {
            log.info("Cleaning up the Message Broker B[Azure] OpenMRSConsumer Connection.");
            try {
                ...
                log.error("Message Broker B[Azure] OpenMRSConsumer connection is not created. {}", e.getMessage());
            }
        }
    }
